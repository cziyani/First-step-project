...
#List all the .gen files and store them in one list for RAD_sub
files <- Sys.glob("./*.gen" )
...
colnames(y) <- c("Sim_ID","NB_RAD_FRAG","Replicate" )
  
## use 3 cores of the computer
registerDoParallel(cores = 3) 
foreach(file=1:length(files) ,.packages = "RZooRoH") %dopar% {
  ##read the data using "zoodata" function
  data_Rohs<- zoodata(files[file], zformat = "gp", samplefile = "./output_Sim1669119579366_160000_10.sample")
  ##choose the model
  Mod5R <- zoomodel(K=6)
  ##find the HBD locus with zoorun function
  loc_mod5r <- zoorun(Mod5R, data_Rohs, localhbd = TRUE)
  ##the local autozygosity estimation/ estimate realized inbreeding by "@realized"
  ...
  loc_table <- loc_mod5r@hbdseg
    ...
    Sim_ID <- strsplit(files[file] , split="_")[[1]][2]
    #extract just the NB_RAD_FRAG from each file name
    NB_RAD_FRAG <- strsplit( files[file], split = "_")[[1]][3]
    #extract just the number of Replicate from each file name by deleting the "."
    Replicate1 <- strsplit(files[file] , split = "[.]")[[1]][2]
